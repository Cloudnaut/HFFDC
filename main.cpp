...
int main(int argc, char *argv[])
{
	...
	else if (IsString(argv[1], "-cmpcpy") || IsString(argv[1], "--compare-and-copy")) //Compare and copy 
	{
		char *fromFilePath = argv[2];
		...
		_sopen_s(&toFileHandle, toFilePath, _O_RDWR | _O_BINARY | _O_CREAT, _SH_DENYNO, _S_IREAD | _S_IWRITE);
		...
		if (fromFileSize < 0 || toFileSize < 0) //File read error
			exit(EXIT_FAILURE);
...
void PrintHelp()
{
	...
	cout << '\t' << "[PARAMETERS: filePath blockSizeInBytes]" << endl;
	cout << '\t' << "Generates a checksum file for the specified file" << endl << endl;

	cout << "-cmpcpy, --compare-and-copy" << endl;
	cout << '\t' << "[PARAMETERS: sourceFilePath targetFilePath blockSizeInBytes]" << endl;
